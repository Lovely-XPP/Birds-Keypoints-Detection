...
def get_dataset_instances_meta():
    ...
    if keypt:
        ...
        for i in range(0, 1 + int(len(skeleton)/2), 2):
            keypoint_connection_rules.append([DATASET_CATEGORIES[skeleton[i][0] + 1]["name"], DATASET_CATEGORIES[skeleton[i][1] + 1]["name"], skeleton[1 + i]])
        ret = {
